...
.orderbook.prevprice:{[exch;pairs]
    price1D: select price1D: (last ask1 + last bid1) % 2 by sym from orderbooktop where exchangeTime < .z.z - 24:00, exchange=exch, sym in pairs;
    price7D: select price7D: (last ask1 + last bid1) % 2 by sym from orderbooktop where exchangeTime < .z.z - 24:00, exchange=exch, sym in pairs;
    price1D^price7D
    }
