...
def run():
    ...
    while True:
        ...
        for currency in currencies:
            ...
            api_id = model_to_dict(CoinGeckoCurrencyIDs.objects.filter(currency=ticker)[0])["api_id"]
            ...
            currency_info = CurrencyInformation(
                ...
                transactions_per_second=(float(blockchair_data["transactions_24h"]) / (24.0 * 60.0 * 60.0)),
                total_transactions=(blockchair_data["transactions"] if "transactions" in blockchair_data else nan),
                market_dominance_percentage=(blockchair_data["market_dominance_percentage"] if "market_dominance_percentage" in blockchair_data else nan),
                active_addresses=(blockchair_data["hodling_addresses"] if "hodling_addresses" in blockchair_data else nan),
                transactions_24h=(blockchair_data["transactions_24h"] if "transactions_24h" in blockchair_data else nan),
                average_transaction_fee_usd_24h=(blockchair_data["average_transaction_fee_usd_24h"] if "average_transaction_fee_usd_24h" in blockchair_data else nan),
